...
def main(args):
    ...
    if CONFIG.fbanet:
        polar_burst_model = model.FBAnet_wrap().to(CONFIG.device[0])


    polar_burst_model.load_state_dict(checkpoint['model_state_dict'])
    polar_burst_model.eval()

    polar_image_sampler = dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_gt,CONFIG.burst_size_val)
