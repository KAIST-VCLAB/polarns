...
def safe_invert_gains(image, rgb_gain, red_gain, blue_gain):
    ...
    assert image.dim() == 3 and image.shape[0] == 3
    device = image.device
    gains = torch.tensor([1.0 / red_gain, 1.0, 1.0 / blue_gain],device=device) / rgb_gain
    gains = gains.view(-1, 1, 1)
...
def mosaic_polar(image):
    """Extracts RGGB Bayer planes from an polar RGB image."""
    shape = image.shape
    if image.dim() == 3:
        image = image.unsqueeze(0)
    stacked_images = [
...
def mosaic_polar_raw(image):
    """Extracts RGGB Bayer planes to raw image from an polar RGB image."""
    shape = image.shape
    if image.dim() == 3:
        image = image.unsqueeze(0)
    ...
    if len(shape) == 3:
        return image.view((1, shape[-2], shape[-1]))
    else:
        return image.view((-1, 1, shape[-2], shape[-1]))
