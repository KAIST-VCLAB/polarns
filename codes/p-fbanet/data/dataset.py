...
class PolarSavedImageNpyDatasetFBANet(SavedImageDataset):
    """
    Dataset for cropped polarization dataset
    input : float32 npy gt images (s0,s1,s2) + 16bit mosaic burst images
    output : float32 tensor images dict frame (D C H W), gt (C H W)
    ...
    def __getitem__(self, idx):
        ...
        frames = []
        for idx_burst in range(self.burst_size):
            frame = imread_polar_mono2bayer_tensor(f'{self.path_each_image[idx]}/frame_{idx_burst:02d}.png')
            # demosaic the frame
            demosaiced_frames = []
            ...
            frames.append(demosaiced_frames)
        frames = torch.stack(frames, dim=0)
        frames = frames ** (1.0 / 2.2)
        frames = align_images(frames)
        frames = frames ** (2.2)

        return {'frames':frames,'gt':gt}


class PolarSavedImageNpyDatasetFBANetCache(PolarSavedImageNpyDatasetFBANet):
    def __init__(self, path_images, burst_size, max_size=None):
        super().__init__(path_images, burst_size)
        self.cache_image = [None]*super().__len__()
        self.max_size = max_size
