...
def train_thread_f(trainer):
    ...
    while (trainer.join_signal is False) or (trainer.queue_job.empty() is False):
        ...
        loss = trainer.loss_fn(sampler.cut_boundary(pred,trainer.bd_size), gt)
...
def test_loop(dataloader, model, loss_fn,device,bd_size,rank,output_format):
    ...
    mse_loss_aolp = 0
    bias_entire_dop = 0
    mse_f = mse_for_properties
    with torch.no_grad():
        for item in dataloader:
            input = item['frames'].to(device)
            pred = model(input)
            pred = sampler.cut_boundary(pred,bd_size)
            gt = sampler.cut_boundary(item['gt'],bd_size).to(device)

            if output_format == 'degree':
                gt_ = stokes2degree(gt)
            elif output_format == 'properties':
                gt_ = stokes2properties(gt)
            else:
                gt_ = gt
            test_loss += loss_fn(pred, gt_)*len(item['gt'])
            if output_format == 'degree':
                pred = degree2stokes(pred)
            if output_format != 'properties':
                pred = stokes2properties(pred)
            gt = stokes2properties(gt)
            mse_s0,mse_dop,bias_dop,mse_aolp = mse_f(pred, gt)
...
def train(rank, world_size, CONFIG):
    ...
    elif CONFIG.image_type == 'polar_real':
        image_dataset = dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_real_train, CONFIG.burst_size[0])
        image_dataset_val = dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_real_val, CONFIG.burst_size_val)
        sampler_func = sampler.RandomIndexing
        info_bursts = CONFIG.info_bursts_real

    print('init model')
    if CONFIG.fbanet:
        denoiser_model = model.FBAnet_wrap().to(CONFIG.device[rank])
    ...
    if not CONFIG.debug:
        image_sampler_val = dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_val_cropped, CONFIG.burst_size_val)

    image_sampler.burst_size = CONFIG.burst_size[0]
    if CONFIG.image_type == 'polar':
        image_sampler_val_image = dataset.ValImageDataset(
            dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_val_cropped, CONFIG.burst_size_val),
            CONFIG.polar_val_image_idx_list)
    if CONFIG.image_type == 'polar_real':
        image_sampler_val_image = dataset.ValImageDataset(
            dataset.PolarSavedImageNpyDatasetFBANetCache(CONFIG.path_image_real_val, CONFIG.burst_size_val),
    ...
    if world_size>1:
        dist.barrier()
        print('DDP init')
        denoiser_model = DDP(denoiser_model, device_ids=[CONFIG.device[rank]])
    ...
    # denoiser_model.module.bsrt.requires_grad_(False)
    
    # if world_size>1:
    #     denoiser_model.module.Float()
    # else:
    #     denoiser_model.Float()
    ...
    for t in range(CONFIG.init_epoch,CONFIG.epochs):
        ...
        if (t + 1)%CONFIG.period_valid==0:
            test_loss,mse_loss_s0,mse_loss_dop,bias_dop,mse_loss_aolp\
                = test_loop(val_dataloader, denoiser_model, loss_fn,CONFIG.device[rank],CONFIG.bd_size,rank,CONFIG.output_format)
...
def main(args):
    ...
    if CONFIG.save_model_code:
        ...
        shutil.copy('./train.py',f'{path_save_codes}')
