...
from PolarDataset import SyntheticPolarBurstVal
...
parser.add_argument('--input_dir', default='/root/data/polar/polar_burst/test',
...
test_dataset = SyntheticPolarBurstVal(root=args.input_dir)
...
with torch.no_grad():
    ...
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        rgb_noisy = data_test['LR'].cuda() # [B=1, 14, 16, H, W]
        rgb_noisy = rgb_noisy.reshape(rgb_noisy.shape[0], rgb_noisy.shape[1], 3, 4, rgb_noisy.shape[3], rgb_noisy.shape[4])
        rgb_noisy = rgb_noisy.squeeze(0).permute(2, 0, 1, 3, 4).contiguous()
