...
import model.FBANet.model as m
...
def polar_demosaic_each(img,pattern='rggb'):
    img_0 = demosaic(img[:,0::4,:,:],pattern='rggb')
    img_45 = demosaic(img[:,1::4,:,:],pattern='rggb')
    img_90 = demosaic(img[:,2::4,:,:],pattern='rggb')
    img_135 = demosaic(img[:,3::4,:,:],pattern='rggb')

    return torch.concat([img_0,img_45,img_90,img_135],1)
...
class FBAnet_wrap(nn.Module):
    def __init__(self):
        super().__init__()
        self.model_restoration = m.BaseModel(img_size=48,embed_dim=64,win_size=8,token_projection='linear',token_mlp='leff')
    
    def forward(self, x):
        B, D, C, H, W = x.shape
        if C== 16:
            x = rearrange(x,'B D C H W -> (B D) C H W',B=B,D=D)
            x = polar_demosaic_each(x)
            x = rearrange(x,'(B D) C H W -> B D C H W',B=B,D=D)
        x = self.model_restoration(x)
        return x
