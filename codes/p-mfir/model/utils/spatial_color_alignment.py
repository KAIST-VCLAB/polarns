...
def match_colors(im_ref, im_q, im_test, ksz, gauss_kernel):
    ...
    im_t_conv = []
    for i in range(4):
        im_test_ch = im_test[:,3*i:3*i+3]
        im_test_re = im_test_ch.view(*im_test_ch.shape[:2], -1) # B C H*W
        im_t_conv_ch = torch.matmul(im_test_re.permute(0, 2, 1), c_mat).permute(0, 2, 1)
        im_t_conv_ch = im_t_conv_ch.view(im_test_ch.shape)
        im_t_conv.append(im_t_conv_ch)
    im_t_conv = torch.cat(im_t_conv,dim=1)
