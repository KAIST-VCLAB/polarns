...
import torch
from model.alignment.pwcnet import PWCNet
...
class PWCNetAlignment(nn.Module):
    def __init__(self, train_alignmentnet=False, flow_ds_factor=1.0):
        ...
        alignment_net = PWCNet(load_pretrained=True,
                               weights_path='./model/alignment/pwcnet-network-default.pth')
    ...
    def forward(self, im, reference_id=0):
        ...
        if im.shape[2] == 16:
            im_rgb = torch.stack((im[:, :, 0:4].mean(dim=2), im[:, :, 4:12].mean(dim=2), im[:, :, 12:16].mean(dim=2)), dim=2)
        else:
            raise Exception
