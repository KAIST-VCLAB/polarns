...
import model.layers.blocks as blocks
from model.layers.upsampling import PixShuffleUpsampler
...
class ResPixShuffleConv(nn.Module):
    ...
    def __init__(self, input_dim, init_conv_dim, num_pre_res_blocks, post_conv_dim,
        ...
        self.predictor = blocks.conv_block(post_conv_dim, 12, 1, stride=1, padding=0, batch_norm=False)
...
class ResDecoder(nn.Module):
    """ Simple decoder using residual blocks"""
    def __init__(self, input_dim, init_conv_dim, num_res_blocks, use_bn=False, activation='relu',
                 out_dim=3, padding_mode='zeros'):
        super().__init__()
        self.init_layer = blocks.conv_block(input_dim, init_conv_dim, 3, stride=1, padding=1, batch_norm=use_bn,
        ...
        self.predictor = blocks.conv_block(d_in, out_dim, 1, stride=1, padding=0, batch_norm=False,
                                           activation='none', padding_mode=padding_mode)

    def forward(self, x):
        x = x['fused_enc']
        assert x.dim() == 4
        out = self.res_layers(self.init_layer(x))
