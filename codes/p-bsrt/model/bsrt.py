...
import os
import math
# from model.polar_layers import *
...
class BSRT(nn.Module):
    def __init__(self, nframes=8, img_size=64, patch_size=1, in_chans=3, out_chans=3,
        ...
        self.conv_flow = nn.Conv2d(in_chans//4, 3, kernel_size=3, stride=1, padding=1)
        ...
        # self.polar_fusion_before = PolarFusion(num_feat, 4)
        ...
        # self.polar_fusion_after = PolarFusion(embed_dim, num_heads[1])
        ...
        # polar align
        # self.polar_align = PolarAlign()

        # # freeze all except polar layers
        # super().eval()
        # super().requires_grad_(False)
        # self.polar_fusion_before.train(True)
        # self.polar_fusion_after.train(True)
        # self.polar_align.train(True)
        # self.polar_fusion_before.requires_grad_(True)
        # self.polar_fusion_after.requires_grad_(True)
        # self.polar_align.requires_grad_(True)
    ...
    def forward(self, x, print_time=False):
        # separate polar channels
        # B_, N, C_, H, W = x.size()
        # x = rearrange(x,"B N (C p) H W -> (B p) N C H W", p=4, C=4)
        ...
        #### polar fusion layer
        # before
        # aligned_fea = self.polar_fusion_before(aligned_fea)
        
        # original fusion layer 
        if not self.non_local:
            aligned_fea = aligned_fea.view(B, -1, H, W)
        x = self.lrelu(self.fusion(aligned_fea))

        # after
        # x = self.polar_fusion_after(x)
