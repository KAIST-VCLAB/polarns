...
class BurstM_wrap(nn.Module):
    def __init__(self,img_size,downsample_factor=2):
        super().__init__()
        self.burstM = BurstM.Neural_Warping()
        self.img_size = img_size
        self.output_res = (img_size*downsample_factor*2,img_size*downsample_factor*2) # should be half
    ...
    def forward(self, frames_batch):
        ...
        for i in range(len(frames_batch)):
            burst = frames_batch[i]
            burst_ref = burst[0].unsqueeze(0).clone()
            burst_src = burst[1:]
            
            burst_feat, ref, EstLrImg = self.burstM(burst_ref, burst_src, self.downsample_factor, self.output_res)
            output.append(burst_feat)
