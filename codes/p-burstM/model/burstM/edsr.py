...
class EDSR(nn.Module):
    ...
    # def __init__(self, args, conv=default_conv):
        ...
        # define head module
        m_head = [default_conv(args.input_channel, n_feats, kernel_size)]
        
        # define body module
        if conv=='default_conv':
            conv = default_conv
            m_body = [
                ResBlock(
...
def make_edsr_baseline(n_resblocks=16, n_feats=64, res_scale=1, n_colors=3,
                       scale=2, no_upsampling=False, rgb_range=1, input_channel=4, conv='default_conv'):
    args = Namespace()
