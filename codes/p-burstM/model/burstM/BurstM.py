...
class Neural_Warping(nn.Module):
    def __init__(self, encoder_spec='edsr-baseline', blender_spec='edsr-baseline', hidden_dim=128):
        ...
        encoder_spec = {'name':'edsr-baseline',
                        'args': {'no_upsampling':True, 'input_channel':16, 'n_resblocks':16, 'n_feats': 128, 'res_scale':1, 'conv': 'default_conv'}}
        ...
        self.encoder = edsr.make_edsr_baseline(**encoder_spec['args'])
        self.blender = edsr.make_edsr_baseline(**blender_spec['args'])
        ...
        # offset generation
        self.fnetp = Fnet.FNetp(in_dim=32)
        self.imnet = nn.Sequential(
            ...
            nn.Conv2d(256, 256, 1, 1, 0),
            nn.ReLU(True),
            nn.Conv2d(256, 48, 1, 1, 0),
        )
        self.lrelu = nn.LeakyReLU(0.1, inplace=True)
        self.skipup1 = nn.Conv2d(4, 128, 3, 1, 1, bias=True)
        self.skipup2 = nn.Conv2d(128, 12 * 4, 3, 1, 1, bias=True)
